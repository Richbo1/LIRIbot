...
function band(inputs){
	var queryUrl="https://rest.bandsintown.com/artists/" + inputs + "/events?app_id=codingbootcamp";
	request(queryUrl, function(error, response, body){
		if(!error && response.statusCode == 200){
		var fbody = JSON.parse(body);
		for(i in fbody){
			console.log(fbody[i].venue.name)
			console.log(fbody[i].venue.city+" "+fbody[i].venue.region)
			console.log(moment(fbody[i].datetime).format("MMM Do YY")); 
		}
		}
	})
}
